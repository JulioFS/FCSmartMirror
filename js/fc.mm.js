...
const WEATHER_IMG_LOC = 'https://openweathermap.org/img/wn';
...
var MM = (function () {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                navigator.geolocation.getCurrentPosition((position) => {
                    ...
                    resolve({ lat: lat.toFixed(2), long: long.toFixed(2) });
                }, (err) => {
                    reject(geoError(err));
                });
            } catch (e) {
    ...
    let _retrieveWeatherData = (lat, long, APPID) => {
        let url = WEATHER_LOC + `?lat=${lat}&lon=${long}&exclude=hourly,minutely&appid=${APPID}`;
        if (APPID.length > 0) {
            return $.ajax({
                url: url,
                type: 'GET'
            ...
            /* Somewhat code smell, just want to be able to return a (rejected) promise to the caller.. */
            return new Promise((resolve, reject) => {
                reject('Weather data cannot be retrieved without a missing APPID.');
            });
        }
    };

    let _renderWeatherData = (wData) => {
    ...
    /* "Crude" Error handler */
    let _weatherDataError = (err, request) => {
        console.error('ERROR Retrieving Weather Data. Error Reported is:');
        console.error(err);
        console.log(request || 'No HTTP Request Available to log.');
        $('#weather-error').removeClass('d-none');
        //alert(`An Error has occurred. Please check the console log for details.\n\n${err}`);
    };

    let _convertKTemp = (kelvin, to) => {
        /* Results come in Kelvin degrees, convert to C or F appropriately */
        let result = kelvin;
        if (to === 'F') {
            /* K to F */
    ...
    let init = async () => {
        if (DEMO_MODE) {
            ...
            try {
                let latLong = await _getLatLong();
                SESSION.lat = latLong.lat;
                SESSION.long = latLong.long;
                let wData = await _retrieveWeatherData(SESSION.lat, SESSION.long, APPID);
                _renderWeatherData(wData);
            } catch (err) {
                _weatherDataError(err);
            }
        }
    };
