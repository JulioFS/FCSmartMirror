...
const WEATHER_LOC = 'https://api.openweathermap.org/data/2.5/onecall?lat=$lat&lon=$lon&exclude=hourly,minutely&appid=' + APPID;
...
var MM = (function () {
    ...
    _retrieveWeatherData = (callBack, errCallBack) => {
        /* POC/Test, make the AJAX call here */
        if (DEMO_MODE) {
    ...
    init = () => {
        ...
        _getLatLong((lat, long) => {
            SESSION.lat = lat;
            SESSION.long = long;
            _retrieveWeatherData(_renderWeatherData, _weatherDataError);
        });
    };
